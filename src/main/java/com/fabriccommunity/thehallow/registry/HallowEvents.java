	...
	public static void init() {
		WitchTickCallback.EVENT.register((witch) -> {
			if (!witch.hasStatusEffect(StatusEffects.WATER_BREATHING)) {
				...
				} else if (witch.checkWaterState() && !witch.updateMovementInFluid(HallowFluidTags.WITCH_WATER)) {
					witch.damage(DamageSource.DROWN, 4.0F);
				}
			}
		...
		LootTableLoadingCallback.EVENT.register(((resourceManager, lootManager, id, supplier, setter) -> {
			if (id.equals(EntityType.WITCH.getLootTableId())) {
				supplier.withPool(FabricLootPoolBuilder.builder().withCondition(RandomChanceLootCondition.builder(0.01f)).withEntry(ItemEntry.builder(HallowItems.GOLDEN_CANDY_CORN)));
			}
		}));
	}
}
