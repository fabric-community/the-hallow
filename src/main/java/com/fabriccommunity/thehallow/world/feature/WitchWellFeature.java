...
public class WitchWellFeature extends Feature<DefaultFeatureConfig> {
	private static final BlockStatePredicate CAN_GENERATE = BlockStatePredicate.forBlock(HallowBlocks.DECEASED_GRASS_BLOCK);
	...
	public WitchWellFeature(Function<Dynamic<?>, ? extends DefaultFeatureConfig> configDeserializer) {
		...
		this.slab = HallowBlocks.TAINTED_SANDSTONE_SLAB.getDefaultState();
		this.wall = HallowBlocks.TAINTED_SANDSTONE.getDefaultState();
		this.fluid = HallowBlocks.WITCH_WATER_BLOCK.getDefaultState();
		this.lantern = Blocks.LANTERN.getDefaultState().with(LanternBlock.HANGING, true);
	}
	
	private BlockState getPumpkin(Direction facing) {
		return HallowBlocks.TINY_PUMPKIN.getDefaultState().with(TinyPumpkinBlock.FACING, facing);
	}
