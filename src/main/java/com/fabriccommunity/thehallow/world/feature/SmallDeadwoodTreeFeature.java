...
public class SmallDeadwoodTreeFeature extends AbstractTreeFeature<DefaultFeatureConfig> {
	private static final BlockState LOG = HallowBlocks.DEADWOOD_LOG.getDefaultState();
	private static final BlockState LEAVES = HallowBlocks.DEADWOOD_LEAVES.getDefaultState();
	...
	protected static boolean isNaturalDirt(TestableWorld world, BlockPos pos) {
		return world.testBlockState(pos, (state) -> {
			...
			return block == HallowBlocks.DECEASED_DIRT;
		});
	}
	...
	protected static boolean isNaturalDirtOrGrass(TestableWorld world, BlockPos pos) {
		return world.testBlockState(pos, (state) -> {
			Block block = state.getBlock();
			return block == HallowBlocks.DECEASED_DIRT || block == HallowBlocks.DECEASED_GRASS_BLOCK;
		});
	}
	...
	private void makeVines(ModifiableTestableWorld world, BlockPos pos, BooleanProperty boolProp) {
		BlockState state = HallowBlocks.DEADWOOD_VINES.getDefaultState().with(boolProp, true);
	...
	protected void setToDirt(ModifiableTestableWorld world, BlockPos pos) {
		if (!isNaturalDirt(world, pos)) {
			this.setBlockState(world, pos, HallowBlocks.DECEASED_DIRT.getDefaultState());
		}
	}
}
