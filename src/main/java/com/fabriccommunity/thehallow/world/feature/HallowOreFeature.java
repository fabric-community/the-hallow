...
public class HallowOreFeature extends Feature<HallowOreFeatureConfig> {
	public HallowOreFeature(Function<Dynamic<?>, ? extends HallowOreFeatureConfig> function_1) {
		super(function_1);
	}
	
	@Override
	public boolean generate(IWorld world, ChunkGenerator<? extends ChunkGeneratorConfig> chunkGenerator, Random random, BlockPos blockPos, HallowOreFeatureConfig oreFeatureConfig) {
	...
	protected boolean generateVeinPart(IWorld world, Random random, HallowOreFeatureConfig oreFeatureConfig, double positiveX, double negativeX, double positiveZ, double negativeZ, double double_5, double double_6, int startX, int yPosition, int startZ, int xSize, int int_5) {
		int stonesPlaced = 0;
								...
								for (int z = int_13; z <= int_16; ++z) {
									...
									if (double_20 * double_20 + double_21 * double_21 + double_22 * double_22 < 1.0D) {
										...
										if (!bitSet.get(int_20)) {
											bitSet.set(int_20);
											blockPos.set(x, y, z);
											if (world.getBlockState(blockPos).getBlock() == HallowBlocks.TAINTED_STONE) {
