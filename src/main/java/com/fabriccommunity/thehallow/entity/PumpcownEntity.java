...
public class PumpcownEntity extends CowEntity {
	...
	public boolean interactMob(PlayerEntity player, Hand hand) {
		...
		if (stack.getItem() == Items.SHEARS && this.getBreedingAge() >= 0) {
			...
			if (!this.world.isClient) {
				...
				if (this.world.getDimension().getType() == HallowDimensions.HALLOW) {
			...
			stack.decrement(1);
			ItemStack stew = new ItemStack(HallowItems.PUMPKIN_STEW);
	...
	public PumpcownEntity createChild(PassiveEntity entity) {
		return HallowEntities.PUMPCOWN.create(this.world);
	}
}
