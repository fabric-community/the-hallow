...
public class RestlessCactusEntity extends MobEntityWithAi {
			...
			if (hop > 0) {
				...
				if (hop == 1) {
					...
					for (i = 0; i < getCactusHeight(); i++) {
						world.setBlockState(landPos.up(i), HallowBlocks.RESTLESS_CACTUS.getDefaultState().with(RestlessCactusBlock.AGE, age));
					}
					this.remove();
				}
				hop--;
			} else if (this.random.nextInt(256) == 0 && world.getBlockState(getBlockPos().down()).getBlock() == HallowBlocks.TAINTED_SAND) {
	...
	public float getPathfindingFavor(BlockPos pos, ViewableWorld world) {
		return world.getBlockState(pos.down()).getBlock() == HallowBlocks.TAINTED_SAND ? 10.0F : 1.0F;
	}
	
	@Override
	public void onDeath(DamageSource source) {
		Block.dropStack(this.world, new BlockPos(this), new ItemStack(HallowBlocks.RESTLESS_CACTUS, getCactusHeight()));
	}
}
