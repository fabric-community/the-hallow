...
public class DeceasedGrassBlock extends GrassBlock {
	...
	public void grow(World world, Random random, BlockPos blockPos, BlockState blockState) {
		BlockPos upPos = blockPos.up();
		BlockState grassBlock = HallowBlocks.DECEASED_GRASS_BLOCK.getDefaultState();
	...
	public void onScheduledTick(BlockState blockState, World world, BlockPos blockPos, Random random) {
		if (!world.isClient) {
			if (!canSurvive(blockState, world, blockPos)) {
				world.setBlockState(blockPos, HallowBlocks.DECEASED_DIRT.getDefaultState());
			} else {
				if (world.getLightLevel(blockPos.up()) >= 9) {
					...
					for (int i = 0; i < 4; ++i) {
						...
						if (world.getBlockState(randomPos).getBlock() == HallowBlocks.DECEASED_DIRT && canSpread(defaultState, world, randomPos)) {
