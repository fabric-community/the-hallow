...
public class RestlessCactusBlock extends Block {
	...
	public void onScheduledTick(BlockState state, World world, BlockPos pos, Random random) {
		if (!state.canPlaceAt(world, pos)) {
			...
			if (world.getBlockState(pos.down(1)).getBlock() == HallowBlocks.TAINTED_SAND && random.nextInt(2) == 0) {
				int height = 0;
				int age = 0;
				...
				RestlessCactusEntity entity = new RestlessCactusEntity(HallowEntities.RESTLESS_CACTUS, world);
	...
	public boolean canPlaceAt(BlockState state, ViewableWorld world, BlockPos pos) {
		...
		do {
			if (!iterator.hasNext()) {
				...
				return (block == HallowBlocks.RESTLESS_CACTUS || block == HallowBlocks.TAINTED_SAND) && !world.getBlockState(pos.up()).getMaterial().isLiquid();
			}
