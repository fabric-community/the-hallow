...
public class WitchWaterBubbleColumnBlock extends Block implements FluidDrainable {
	...
	public static void update(IWorld world, BlockPos pos, boolean bool) {
		if (isStillWater(world, pos)) {
			world.setBlockState(pos, HallowBlocks.WITCH_WATER_BUBBLE_COLUMN.getDefaultState().with(DRAG, bool), 2);
		}
	}
	
	public static boolean isStillWater(IWorld world, BlockPos pos) {
		FluidState state = world.getFluidState(pos);
		return world.getBlockState(pos).getBlock() == HallowBlocks.WITCH_WATER_BLOCK && state.getLevel() >= 8 && state.isStill();
	}
	
	private static boolean calculateDrag(BlockView view, BlockPos pos) {
		BlockState state = view.getBlockState(pos);
		Block block = state.getBlock();
		if (block == HallowBlocks.WITCH_WATER_BUBBLE_COLUMN) {
			return state.get(DRAG);
		} else {
			return block != HallowBlocks.TAINTED_SAND;
		}
	}
	...
	public FluidState getFluidState(BlockState state) {
		return HallowFluids.WITCH_WATER.getStill(false);
	}
	...
	public BlockState getStateForNeighborUpdate(BlockState state, Direction dir, BlockState state2, IWorld world, BlockPos pos, BlockPos pos2) {
		if (!state.canPlaceAt(world, pos)) {
			return HallowBlocks.WITCH_WATER_BLOCK.getDefaultState();
		} else {
			if (dir == Direction.DOWN) {
				world.setBlockState(pos, HallowBlocks.WITCH_WATER_BUBBLE_COLUMN.getDefaultState().with(DRAG, calculateDrag(world, pos2)), 2);
			} else if (dir == Direction.UP && state2.getBlock() != HallowBlocks.WITCH_WATER_BUBBLE_COLUMN && isStillWater(world, pos2)) {
				world.getBlockTickScheduler().schedule(pos, this, this.getTickRate(world));
			}
			world.getFluidTickScheduler().schedule(pos, HallowFluids.WITCH_WATER, HallowFluids.WITCH_WATER.getTickRate(world));
	...
	public boolean canPlaceAt(BlockState state, ViewableWorld world, BlockPos pos) {
		...
		return block == HallowBlocks.WITCH_WATER_BUBBLE_COLUMN || block == HallowBlocks.BLEEDING_BLOCK || block == HallowBlocks.TAINTED_SAND;
	}
	...
	public Fluid tryDrainFluid(IWorld world, BlockPos pos, BlockState state) {
		...
		return HallowFluids.WITCH_WATER;
	}
}
